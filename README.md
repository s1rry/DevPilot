...
> **Статус:** активная открытая разработка. Готовые установщики доступны на странице [Releases](https://github.com/s1rry/devpilot/releases) — либо соберите [из исходников](#установка-из-исходников).
...
Скачайте установщик для своей ОС со страницы [**Releases**](https://github.com/s1rry/devpilot/releases/latest):

| ОС | Файл | Как поставить |
|---|---|---|
| **Windows** | `DevPilot_*_x64-setup.exe` (или `.msi`) | Скачать, запустить, «Далее» — готово. |
| **macOS** | `DevPilot_*_universal.dmg` | Один файл для Apple Silicon и Intel. Открыть образ, перетащить в «Программы». |
| **Linux** | `DevPilot_*_amd64.AppImage` | Сделать исполняемым (`chmod +x`) и запустить. Также есть `.deb` и `.rpm`. |

Приложение не требует прав администратора и работает автономно.

**Обновления.** Начиная с версии 0.2.4 DevPilot обновляется сам: при запуске он проверяет наличие новой версии и предлагает установить её в один клик (обновления подписаны). Файлы `.sig`, `latest.json` и `.app.tar.gz` на странице Releases нужны этому механизму — вручную их скачивать не нужно.

## Первый запуск

Сборки пока **без цифровой подписи**, поэтому при первом запуске система покажет предупреждение. Это нормально — приложение полностью открыто, исходники в этом репозитории. Как открыть:

**macOS.** Gatekeeper скажет, что разработчик не подтверждён. Обход в один раз:

- Нажмите на приложение **правой кнопкой** (или Ctrl+клик) → **«Открыть»** → в диалоге снова **«Открыть»**. Дальше запускается как обычно.
- Если не помогло, снимите карантин в терминале:
  ```sh
  xattr -dr com.apple.quarantine /Applications/DevPilot.app
  ```

**Windows.** SmartScreen покажет синее окно «Windows защитила ваш компьютер». Нажмите **«Подробнее»** → **«Выполнить в любом случае»**.

**Linux.** Предупреждений нет. Для AppImage дайте право на запуск: `chmod +x DevPilot_*_amd64.AppImage`, затем запустите двойным кликом или из терминала.

> Когда появится цифровая подпись (Apple Notarization и Windows code signing), предупреждения исчезнут и этот шаг не понадобится.
